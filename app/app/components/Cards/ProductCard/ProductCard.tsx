        ...
        <Link to={link} className="no-underline">
            ...
            >
                <div className="w-[154px] h-[270px] rounded-[14px] bg-[#f3f3f3] relative overflow-hidden mb-[10px]">
                    <img
                        src={media.image}
                        className={`absolute inset-0 h-full w-full border-none rounded-[14px] object-cover transition-opacity duration-300 ${isHovered && isMediaLoaded ? 'opacity-0' : 'opacity-100'}`}
                        alt={name}
                    />
                    {media.hover && (
                        mediaType === "video" ? (
                            <video
                                src={media.hover}
                                className={`absolute inset-0 h-full w-full border-none rounded-[14px] object-cover transition-opacity duration-300 ${isHovered && isMediaLoaded ? 'opacity-100' : 'opacity-0'}`}
                                autoPlay
                                loop
                                muted
                                playsInline
