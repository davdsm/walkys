...
interface CategoryWithProducts extends CategoryRecord {
...
export const CategoriesList = ({ categories, language }: CategoriesListProps) => {
  return (
      ...
      {categories.map((category: CategoryWithProducts, indexC: number) => (
        ...
        >
          ...
          <ul className="flex flex-row gap-3 justify-start items-center flex-wrap md:flex-nowrap">
            {category.products.map((product: ProductRecord, indexP: number) => (
              <motion.li
                key={product.id}
                initial={{ opacity: 0, y: 60 }}
                whileInView={{ opacity: 1, y: 0 }}
