        ...
        <Link to={link} className="no-underline">
            ...
            >
                <div className="mb-[12px] h-[145px] w-full bg-[#f3f3f3] rounded-[10px] relative overflow-hidden">
                    <img
                        src={media.image}
                        className={`absolute inset-0 h-full w-full border-none rounded-[10px] object-cover transition-opacity duration-300 ${isHovered && isMediaLoaded ? 'opacity-0' : 'opacity-100'}`}
                        alt={name}
                    />
                    {mediaType === "video" ? (
                        <video
                            src={media.hover}
                            className={`absolute inset-0 h-full w-full border-none rounded-[10px] object-cover transition-opacity duration-300 ${isHovered && isMediaLoaded ? 'opacity-100' : 'opacity-0'}`}
                            autoPlay
                            loop
                            muted
                            playsInline
