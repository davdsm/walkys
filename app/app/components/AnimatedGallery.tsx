...
const NUM_STEPS = 4;
const STEP_RANGE = 0.18;
const STEP_START = 0.25;
...
export const AnimatedGallery = ({
  ...
  const cardOpacity = useTransform(scrollYProgress, [0.15, 0.2], [0, 1]);

  // Step opacities at top level (no hooks in loop)
  const stepOpacity0 = useTransform(
    scrollYProgress,
    [STEP_START, STEP_START + 0.05, STEP_START + STEP_RANGE - 0.05, STEP_START + STEP_RANGE],
    [0, 1, 1, 0]
  );
  const stepOpacity1 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE, STEP_START + STEP_RANGE + 0.05, STEP_START + STEP_RANGE * 2 - 0.05, STEP_START + STEP_RANGE * 2],
    [0, 1, 1, 0]
  );
  const stepOpacity2 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE * 2, STEP_START + STEP_RANGE * 2 + 0.05, STEP_START + STEP_RANGE * 3 - 0.05, STEP_START + STEP_RANGE * 3],
    [0, 1, 1, 0]
  );
  const stepOpacity3 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE * 3, STEP_START + STEP_RANGE * 3 + 0.05, STEP_START + STEP_RANGE * 4 - 0.05, STEP_START + STEP_RANGE * 4],
    [0, 1, 1, 0]
  );
  const stepOpacities = [stepOpacity0, stepOpacity1, stepOpacity2, stepOpacity3];

  const overlayOpacity1 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE - 0.05, STEP_START + STEP_RANGE],
    [0, 1]
  );
  const overlayOpacity2 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE * 2 - 0.05, STEP_START + STEP_RANGE * 2],
    [0, 1]
  );
  const overlayOpacity3 = useTransform(
    scrollYProgress,
    [STEP_START + STEP_RANGE * 3 - 0.05, STEP_START + STEP_RANGE * 3],
    [0, 1]
  );
  const overlayOpacities = [overlayOpacity1, overlayOpacity2, overlayOpacity3];
  ...
  const stepsToShow = steps.slice(0, NUM_STEPS);
    ...
    <ParallaxProvider>
      <div ref={containerRef} className="relative h-[600vh] w-full md:pt-40 md:pb-12">
        <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center">
          ...
          <motion.div
            style={{ scale: heroScale, zIndex: 20 }}
          ...
          >
            ...
            {stepImages.slice(1, 4).map((img, index) => (
              <motion.div
